...
export class PacienteService {
    ...
    cargarPaciente(
        ...
        barrioPac: string): Promise<Paciente> {
            console.log(this.pacienteURL);
        return this.http.post(this.pacienteURL,
            JSON.stringify({dniPaciente: dniPac, nombrePaciente: nombrePac,
                            apellidoPaciente: apellidoPac, telefonoPaciente: telefonoPac,
                            direccionPaciente: direccionPac,fechaNacimientoPaciente:fechaNacimientoPac,barrioPaciente:barrioPac}), {headers: this.headers})
        .toPromise()
        .then(response => response.json().obj as Paciente)
        .catch(this.handleError);
    }
