...
export class UsuarioService {
    ...
    createUsuario(userName: string, userFirstName: string, userLastName: string, password: string): Promise<Usuario> {
        return this.http
            .post(this.usuarioURL, JSON.stringify({
                username: userName, firstName: userFirstName,
                lastName: userLastName, password: password}), {headers: this.headers})
            .toPromise()
            .then(res => {
                return res.json().obj;
            })
    ...
    public updateUsuario(id: number, userName: string, userFirstName: string, userLastName: string,
                        password: string, permissions: Permission[] ): Promise<Usuario> {
        return this.http
            .put(this.usuarioURL,
                JSON.stringify({_id: id, username: userName, firstName: userFirstName,
                                lastName: userLastName, password: password, permissions: permissions}),
